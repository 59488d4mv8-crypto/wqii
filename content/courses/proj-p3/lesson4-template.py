...
def segment(df):
    ...
    d['F'] = pd.qcut(d['sessions'], 5, labels=list(reversed(labels)), duplicates='drop').astype(int)
    d['M'] = pd.qcut(d['total_learn_minutes'], 5, labels=list(reversed(labels)), duplicates='drop').astype(int)
    # 注意 F/M 原始 labels 用 [1,2,3,4,5]；重新处理以保持 qcut 升序
